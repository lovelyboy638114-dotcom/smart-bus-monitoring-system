...
import { 
  ...
  X, Clipboard, Printer, Download, Key, CheckCircle, UserPlus, Volume2 
} from 'lucide-react';
...
const DriverAnalysis = () => {
  ...
  // Active webcam bus stream (only one bus camera streams laptop webcam to CV engine)
  const [activeWebcamBusId, setActiveWebcamBusId] = useState("TN38AB1234");
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/admin/drivers`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok && resJson.success) {
        ...
        const refreshResponse = await fetch(`${API_BASE_URL}/api/v1/buses`);
      ...
      <div className="flex justify-between items-start gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-wide flex items-center gap-1.5 shadow-sm transition-all cursor-pointer"
            title="Test Cabin Buzzer on Laptop Speakers"
          >
            <Volume2 className="w-3.5 h-3.5" /> Test Buzzer (3s)
          </button>
          <button
        ...
        {buses.map((bus) => (
          <div key={bus.id} className="flex flex-col gap-3">
            <div className="bg-slate-50 p-3.5 border border-slate-150 rounded-xl flex justify-between items-center shadow-inner">
              ...
              <button
                onClick={() => setActiveWebcamBusId(bus.id)}
                className={`px-2 py-0.5 text-[8.5px] font-extrabold rounded-full uppercase tracking-wider transition-all cursor-pointer ${
                  activeWebcamBusId === bus.id
                    ? 'text-emerald-700 bg-emerald-50 border border-emerald-300 ring-2 ring-emerald-400/20'
                    : 'text-slate-500 bg-slate-100 hover:bg-slate-200 border border-slate-200'
                }`}
                title={activeWebcamBusId === bus.id ? 'Laptop webcam currently active on this bus camera' : 'Click to connect laptop webcam to this bus camera'}
              >
                {activeWebcamBusId === bus.id ? '● Active AI Cam' : '○ Switch to Cam'}
              </button>
            </div>
            ...
            <ErrorBoundary>
              <CameraMock 
                ...
                hideSimulators={true}
                isActiveWebcam={bus.id === activeWebcamBusId}
                onSelectActive={() => setActiveWebcamBusId(bus.id)}
              />
            </ErrorBoundary>
          </div>
        ))}
