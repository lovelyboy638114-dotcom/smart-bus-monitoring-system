...
java -Xms16m -Xmx56m $COMMON_JVM_OPTS -jar eureka-server.jar > /tmp/eureka.log 2>&1 &
EUREKA_PID=$!
...
java -Xms16m -Xmx56m $COMMON_JVM_OPTS -jar config-server.jar > /tmp/config.log 2>&1 &
CONFIG_PID=$!
...
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -jar auth-service.jar > /tmp/auth.log 2>&1 &
sleep 2

echo "[5/9] Starting Student Service on port 8082..."
java -Xms16m -Xmx84m $COMMON_JVM_OPTS -jar student-service.jar > /tmp/student.log 2>&1 &
sleep 2

echo "[6/9] Starting Transport Service on port 8083..."
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -jar transport-service.jar > /tmp/transport.log 2>&1 &
sleep 2

echo "[7/9] Starting Attendance Service on port 8084..."
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -jar attendance-service.jar > /tmp/attendance.log 2>&1 &
sleep 2

echo "[8/9] Starting Notification Service on port 8086..."
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -jar notification-service.jar > /tmp/notification.log 2>&1 &
