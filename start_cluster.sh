...
COMMON_JVM_OPTS="-XX:+UseSerialGC -XX:TieredStopAtLevel=1 -Xss256k -XX:MaxMetaspaceSize=96m -XX:CompressedClassSpaceSize=16m -XX:ReservedCodeCacheSize=24m -XX:+ExitOnOutOfMemoryError"
...
java -Xms16m -Xmx56m $COMMON_JVM_OPTS -Dserver.port=8761 -jar eureka-server.jar > /tmp/eureka.log 2>&1 &
EUREKA_PID=$!
...
java -Xms16m -Xmx56m $COMMON_JVM_OPTS -Dserver.port=8888 -jar config-server.jar > /tmp/config.log 2>&1 &
CONFIG_PID=$!
...
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -Dserver.port=8081 -jar auth-service.jar > /tmp/auth.log 2>&1 &
sleep 2

echo "[5/9] Starting Student Service on port 8082..."
java -Xms16m -Xmx84m $COMMON_JVM_OPTS -Dserver.port=8082 -jar student-service.jar > /tmp/student.log 2>&1 &
sleep 2

echo "[6/9] Starting Transport Service on port 8083..."
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -Dserver.port=8083 -jar transport-service.jar > /tmp/transport.log 2>&1 &
sleep 2

echo "[7/9] Starting Attendance Service on port 8084..."
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -Dserver.port=8084 -jar attendance-service.jar > /tmp/attendance.log 2>&1 &
sleep 2

echo "[8/9] Starting Notification Service on port 8086..."
java -Xms16m -Xmx72m $COMMON_JVM_OPTS -Dserver.port=8086 -jar notification-service.jar > /tmp/notification.log 2>&1 &
...
exec java -Xms16m -Xmx84m $COMMON_JVM_OPTS -Dserver.port=${GATEWAY_PORT} -jar api-gateway.jar
