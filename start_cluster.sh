...
# Helper function to wait for a service port to respond
wait_for_port() {
  local port=$1
  local name=$2
  local max_sec=${3:-35}
...
java -Xms8m -Xmx48m $COMMON_JVM_OPTS -Dserver.port=8761 -jar eureka-server.jar > /tmp/eureka.log 2>&1 &
wait_for_port 8761 "Eureka Server" 30

# 2. Start Config Server (Port 8888)
echo "[2/9] Starting Config Server on port 8888..."
java -Xms8m -Xmx48m $COMMON_JVM_OPTS -Dserver.port=8888 -jar config-server.jar > /tmp/config.log 2>&1 &
...
java -Xms8m -Xmx56m $COMMON_JVM_OPTS -Dserver.port=8081 -jar auth-service.jar > /tmp/auth.log 2>&1 &
wait_for_port 8081 "Auth Service" 25

echo "[5/9] Starting Student Service on port 8082..."
java -Xms8m -Xmx64m $COMMON_JVM_OPTS -Dserver.port=8082 -jar student-service.jar > /tmp/student.log 2>&1 &
wait_for_port 8082 "Student Service" 25

echo "[6/9] Starting Transport Service on port 8083..."
java -Xms8m -Xmx56m $COMMON_JVM_OPTS -Dserver.port=8083 -jar transport-service.jar > /tmp/transport.log 2>&1 &
wait_for_port 8083 "Transport Service" 25

echo "[7/9] Starting Attendance Service on port 8084..."
java -Xms8m -Xmx56m $COMMON_JVM_OPTS -Dserver.port=8084 -jar attendance-service.jar > /tmp/attendance.log 2>&1 &
wait_for_port 8084 "Attendance Service" 25

echo "[8/9] Starting Notification Service on port 8086..."
java -Xms8m -Xmx56m $COMMON_JVM_OPTS -Dserver.port=8086 -jar notification-service.jar > /tmp/notification.log 2>&1 &
...
exec java -Xms8m -Xmx64m $COMMON_JVM_OPTS -Dserver.port=${GATEWAY_PORT} -jar api-gateway.jar
